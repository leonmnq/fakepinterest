...
def homepage():
    ...
    if form_login.validate_on_submit():  # se todos os campos estão preenchidos e o usuário clicou no botão Login
        ...
        if usuario:  # se for encontrado esse email no banco de dados
            if bcrypt.check_password_hash(usuario.senha.encode(), form_login.senha.data):  # compara a senha criptografada do bando de dados com a senha que o usuário acabou de digitar
                login_user(usuario)
...
def criar_conta():
    ...
    if form_criarconta.validate_on_submit():  # se todos os campos estão preenchidos e o usuário clicou no botão Criar Conta
        senha = bcrypt.generate_password_hash(form_criarconta.senha.data).decode()  # criptografa a senha
        usuario = Usuario(
